...
export const getters = {
  ...
  getFixtures: state => (team) => {
    return state.fixtures.reduce((accumulator, currentValue) => {
      if (
        currentValue.teams[0].id === team ||
        currentValue.teams[1].id === team
      ) {
...
export const mutations = {
  setYear (state, payload) {
    if (
      (payload.year === '2020' || payload.year === '2021') &&
  ...
  restoreStoreModule (state, { module, value }) {
    ...
    ;(split.length ? split.reduce((acc, cur) => acc[cur], state) : state)[
      last
    ] = value
  }
}
...
export const actions = {
  async getTeams ({ commit, state }) {
    const res = await this.$axios.get(state.year + '/teams.json', {
      progress: false
    })
  ...
  async getFixtures ({ commit, state }) {
    const res = await this.$axios.get(state.year + '/fixtures.json', {
      progress: false
    })
